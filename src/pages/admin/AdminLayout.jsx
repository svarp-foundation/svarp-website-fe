...
const AdminLayout = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-dvh bg-muted font-sans text-primary overflow-x-hidden w-full relative">
      {/* Mobile Top Bar */}
      <div className="md:hidden w-full bg-primary text-white p-4 flex justify-between items-center fixed top-0 left-0 z-40 border-b border-white/5">
        <h2 className="text-xl font-extrabold tracking-tight text-accent">
          SVARP
        </h2>
        <button
      ...
      {/* Sidebar */}
      <aside
        className={`w-64 bg-primary text-white fixed h-dvh border-r border-white/5 flex flex-col z-50 transition-all duration-300
      ...
      >
        <div className="p-8 flex items-center justify-between gap-3">
          ...
          <button
            onClick={() => setIsSidebarOpen(false)}
            className="md:hidden p-1 bg-white/10 hover:bg-white/20 rounded-lg transition-all"
          >
        ...
        <div className="p-6 border-t border-white/5">
          <button
            onClick={logout}
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-red-500/10 text-red-500 border border-red-500/20 rounded-lg font-bold hover:bg-red-500 hover:text-white transition-all group"
          >
      ...
      {/* Main Content */}
      <main className="flex-1 min-w-0 ml-64 p-10 max-w-7xl max-md:ml-0 max-md:p-5 max-md:pt-24 w-full">
        <Outlet />
      </main>
    </div>
  );
